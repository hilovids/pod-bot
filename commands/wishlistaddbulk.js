...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('wishlistaddbulk')
        .setDescription('Upload a TXT file to bulk add cards to your wishlist. Format: "1 Dazzling Denial (BLB) 45"')
        .addAttachmentOption(option =>
            option.setName('file')
                .setDescription('Upload a .txt file containing your wishlist.')
                .setRequired(true)
        ),
    async execute(interaction) {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        console.log(`🟢 Wishlist import started by ${interaction.user.username} (${interaction.user.id})`);
        ...
        try {
            ...
            if (insertData.length > 0) {
                console.log(`📥 Bulk inserting ${insertData.length} cards into wishlist...`);
                await query(
                    `INSERT INTO pod_wishlist (pod_wishid, pod_userid, pod_quantity, pod_scryfallid, pod_oracleid, pod_isfoil) 
                     VALUES ${insertData.map(() => `(UUID(), ?, ?, ?, ?, ?)`).join(", ")} 
                     ON DUPLICATE KEY UPDATE pod_quantity = pod_quantity + VALUES(pod_quantity);`,
                    insertData.flat()
                );                
                console.log(`✅ Wishlist insert completed successfully.`);
            } else {
                console.log(`⚠️ No valid cards to insert.`);
            }

            console.log(`✅ Wishlist import complete: ${processedCount}/${cardEntries.length} cards processed.`);

            let description = `Successfully added **${processedCount}** cards to your wishlist.`;
            let attachment = null;
            let tempFilePath = null;

            if (skippedCards.length > 0) {
                // Create a temp file in the local temp directory
                tempFilePath = path.join(TEMP_DIR, `skipped_wishlist_${discordUser.id}.txt`);
            ...
            const successEmbed = new EmbedBuilder()
                .setColor(0x00FF00)
                .setTitle(`✅ Wishlist Import Successful`)
            ...
            console.error('❌ Error processing wishlist import:', error);
